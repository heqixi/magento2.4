...
class Config implements ConfigInterface
{
    ...
    public function getPageEndpoint(\DragDropr\Magento2\Api\Data\EnvironmentInterface $environment = null)
    {
        if ($environment && $environment->getPageEndpoint()) {
            return $this->applyJwtToken($environment->getPageEndpoint());
        }

        return $this->applyJwtToken(
    ...
    public function getCategoryEndpoint(\DragDropr\Magento2\Api\Data\EnvironmentInterface $environment = null)
    {
        if ($environment && $environment->getCategoryEndpoint()) {
            return $this->applyJwtToken($environment->getCategoryEndpoint());
        }
    ...
    public function getDefaultEndpoint(\DragDropr\Magento2\Api\Data\EnvironmentInterface $environment = null)
    {
        if ($environment && $environment->getDefaultEndpoint()) {
            return $this->applyJwtToken($environment->getDefaultEndpoint());
        }
    ...
    {
        $currentJwtToken = $this->getJwtToken();

        if ($currentJwtToken !== $jwtToken) {
            $this->writer->save(static::JWT_TOKEN, $jwtToken, ScopeConfigInterface::SCOPE_TYPE_DEFAULT);
    ...
    {
        return (string) $this->scopeConfig->getValue(static::JWT_TOKEN, ScopeConfigInterface::SCOPE_TYPE_DEFAULT);
    }
     ...
     * @return string
     */
    public function applyJwtToken($url)
    {
